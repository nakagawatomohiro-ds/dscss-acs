...
import { STAGES, CLASSES, MAX_POINTS, RANKS, getRank, POINTS_PER_CORRECT, QUESTIONS_PER_COURSE } from "@/lib/constants";
...
export default function Home() {
  ...
  useEffect(() => {
    const loadProgress = async () => {
      try {
        // Helper: extract raw score (0-10) from any stored format
        // Old object format: { score: N, completedQuestions: M, finished: bool } → N
        // Old points format: number > 10 → was saved as score * 20, convert back
        // New format: number 0-10 (raw score)
        const toRawScore = (val: any): number => {
          if (typeof val === "number") {
            return val > 10 ? Math.round(val / 20) : val;
          }
        ...
        // First, load from localStorage (convert to raw scores)
        ...
        if (dbData && typeof dbData === "object" && !dbData.error) {
          Object.entries(dbData).forEach(([key, val]: [string, any]) => {
            const dbRaw = toRawScore(val);
            const localRaw = localData[key] ?? 0;
            merged[key] = Math.max(localRaw, dbRaw);
          });
        }

        // Migrate localStorage to new format (raw scores 0-10)
        localStorage.setItem("dscss_acs_progress", JSON.stringify(merged));

        // Calculate total points from raw scores
        const total = Object.values(merged).reduce((sum, raw) => sum + raw * POINTS_PER_CORRECT, 0);
        ...
        // Silent fail - use localStorage only (convert to raw scores)
        const storedProgress = localStorage.getItem("dscss_acs_progress");
        const rawLocal = storedProgress ? JSON.parse(storedProgress) : {};
        const localData: ProgressData = {};
        Object.entries(rawLocal).forEach(([key, val]) => {
          const raw = typeof val === "number"
            ? (val > 10 ? Math.round(val / 20) : val)
            : (val && typeof val === "object" && typeof (val as any).score === "number")
              ? (val as any).score : 0;
          localData[key] = raw;
        });
        const total = Object.values(localData).reduce((sum, raw) => sum + raw * POINTS_PER_CORRECT, 0);
  ...
  const completedCount = Object.values(progress).filter((raw) => raw > 0).length;
  const totalCourses = STAGES.length * CLASSES.length;
  const achievementRate = Math.round((completedCount / totalCourses) * 100);
  const totalRawScore = Object.values(progress).reduce((sum, raw) => sum + raw, 0);
  const accuracyRate = completedCount > 0 ? Math.round((totalRawScore / (completedCount * QUESTIONS_PER_COURSE)) * 100) : 0;
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ({completedCount}/{totalCourses}コース)
                    </span>
                  </div>
                </div>
                  ...
                  >
                    <span
                      style={{
                        fontSize: 36,
                        fontWeight: "bold",
                        color: accuracyRate >= 80 ? "#16a34a" : accuracyRate >= 60 ? "#ca8a04" : completedCount > 0 ? "#ef4444" : "#94a3b8",
                      }}
                    >
                      {completedCount > 0 ? `${accuracyRate}%` : "—"}
                    </span>
                    {completedCount > 0 && (
                      <span
                        style={{
                      ...
                      >
                        ({totalRawScore}/{completedCount * QUESTIONS_PER_COURSE}問)
                      </span>
                    )}
                  </div>
                </div>
